...
static int avf_read_header(AVFormatContext *s)
{
    ...
    if (ctx->list_devices) {
        ...
        for (AVCaptureDevice *device in devices) {
            ...
            if (s->ffCapDeviceInfoCallback.callback) {
                s->ffCapDeviceInfoCallback.callback(s->ffCapDeviceInfoCallback.opaque, 0, name);
            }
            index++;
        }
#if !TARGET_OS_IPHONE && __MAC_OS_X_VERSION_MIN_REQUIRED >= 1070
        if (num_screens > 0) {
            ...
            for (int i = 0; i < num_screens; i++) {
                ...
                char captureScreenName[1024];
                sprintf(captureScreenName, "Capture screen %d", i);
                if (s->ffCapDeviceInfoCallback.callback) {
                    s->ffCapDeviceInfoCallback.callback(s->ffCapDeviceInfoCallback.opaque, 0, captureScreenName);
                }
            }
        }
#endif
        ...
        for (AVCaptureDevice *device in devices) {
            ...
            if (s->ffCapDeviceInfoCallback.callback) {
                s->ffCapDeviceInfoCallback.callback(s->ffCapDeviceInfoCallback.opaque, 1, name);
            }
        }
         goto fail;
    }
